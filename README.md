...
The `extra_groups` setting allows relaying additional multicast streams generated by ka9q-radio beyond the default `status_group` and `data_group`. This is useful when radiod is configured with per-band IQ demodulator sections, each producing its own multicast stream.

Each entry matches a `data =` value from a `[section]` in `radiod.conf`:

```ini
# radiod.conf excerpt
[WEBSDR-80m]
data = websdr80m.local
mode = iq
...
```

```yaml
# config.yaml — relay that stream
multicast_relay:
  extra_groups:
    - websdr80m.local:5004
```
